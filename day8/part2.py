...
def link_junction_boxes(junction_boxes: list[JunctionBox]) -> tuple[JunctionBox, JunctionBox]:
    ...
    while(True):
        ...
        iteration += 1
        if len(circuits) == 1 and circuits[0].count_junction_boxes() == len(junction_boxes):
            return (jb1, jb2)

def main():
    junction_boxes = parse_data("input.txt")
    last_pair = link_junction_boxes(junction_boxes)

    print("Last pair mult X distance: ", last_pair[0].coordinate.x * last_pair[1].coordinate.x)
